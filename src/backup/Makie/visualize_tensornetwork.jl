...
function update_lines(adjlist::Vector{Vector{Int}}, points::Vector{Point2f0})
...
function endpoint_nodesizes(adjlist::Vector{Vector{Int}}, nodesizes::Vector{<:Real})
...
# for use with `arrows!`
function lines_to_arrows(
  lines::Vector{Pair{Point2f0,Point2f0}}; shorten_length=fill(0.0, length(lines))
)
...
function lines_to_edgelabelpoints(
  lines, shiftedgelabels=fill(Point2f0(0, 0), length(lines))
)
...
function visualize_network!(
  ...
  if showarrows
    ...
    start, dirs =
      Node.(
        lines_to_arrows(
          lines[];
          shorten_length=endpoint_nodesizes(adjlist, nodesizes) ./ shorten_arrow_scale,
        ),
      )
    ...
    for nodeᵢ in 1:length(adjlist)
      for nodeⱼ in adjlist[nodeᵢ]
        ...
        while haskey(edgelabels, (nodeᵢ, nodeⱼ, nrepeat)) &&
          !haskey(label_already_used, (nodeᵢ, nodeⱼ, nrepeat))
          if nrepeat > 1
            edgelabel *= "\n ⊗ "
          end
        ...
        nrepeat = 1
        while haskey(edgelabels, (nodeⱼ, nodeᵢ, nrepeat)) &&
          !haskey(label_already_used, (nodeⱼ, nodeᵢ, nrepeat))
          edgelabel *= "\n ⊗ "
        ...
        text!(
          scene,
          edgelabel;
          textsize=edgelabelsize,
          position=edgelabelpoints[n],
  ...
  function add_move!(scene, points, pplot)
    idx = Ref(0)
    dragstart = Ref(false)
    startpos = Base.RefValue(Point2f0(0))
...
function visualize_tensornetwork(
  As::ITensor...;
  ...
  if length(As) ≠ length(labels)
    println(
      "Number of tensor labels $labels does not match the number of tensors $(length(As)). Setting default values instead.",
    )
    labels = tensor_labels(length(As))
  end
