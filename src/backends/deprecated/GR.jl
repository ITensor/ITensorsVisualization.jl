using GRUtils: GRUtils

function clear_axis!(plot)
  GRUtils.grid!(plot, false)
  GRUtils.xticklabels!(plot, String[""])
  GRUtils.yticklabels!(plot, String[""])
  GRUtils.xticks!(plot, 0)
  return GRUtils.yticks!(plot, 0)
end

function plot(::Backend"GR"; xlim, ylim, width, height)
  plot = GRUtils.plot([])
  clear_axis!(plot)
  return plot
end

translate_color(::Backend"GR", ::Val{:blue}) = "b"

function draw_edge!(backend::Backend"GR", plot, v1, v2; color)
  gr_color = translate_color(backend, Val(color))
  x, y = point_to_line(v1, v2)
  GRUtils.oplot!(plot, x, y, gr_color) #; kwargs...)
  clear_axis!(plot)
  return plot
end

function annotate!(::Backend"GR", plot, x, y, str)
  GRUtils.annotations!(plot, x, y, str)
  clear_axis!(plot)
  return plot
end

## function lineplot!(backend::Backend"GR", plot, f::Function, x1, x2; kwargs...)
##   r = range(x1, x2; length=100)
##   lineplot!(backend, plot, r, f.(r)) #; kwargs...) 
##   return plot
## end
